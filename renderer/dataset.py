...
class TFDataset(Dataset):
    def __init__(self, root_dir, split='train'):
        ...
        # Detect optional SMIRK pose directory (same naming convention as video_frame/lmd)
        self._smirk_root = self._find_smirk_root()
        self.has_smirk = self._smirk_root is not None
        if not self.has_smirk:
            print("[TFDataset] SMIRK pose data not found — pose supervision disabled.")
    ...
    def _find_smirk_root(self):
        """Return the SMIRK directory if it exists alongside the dataset, else None."""
        candidate = self.root_path / "smirk"
        if candidate.is_dir():
            return candidate
        # Fallback: {parent}/{name}_smirk  (mirrors the _video_frame / _lmd convention)
        candidate = self.root_path.parent / f"{self.root_path.name}_smirk"
        if candidate.is_dir():
            return candidate
        return None
    ...
    def _load_metadata(self):
        ...
        # 90/10 train/val split — works for any dataset size
        split_idx = max(1, int(len(clip_dirs) * 0.9))
        if self.split == 'train':
        ...
        for clip_path in tqdm(clip_dirs, desc=f'Processing {self.root_path.name}'):
            ...
            smirk_path = None
            if self._smirk_root is not None:
                sp = self._smirk_root / f"{clip_path.name}.pt"
                smirk_path = str(sp) if sp.is_file() else None
    ...
    def __getitem__(self, idx):
        ...
        # Optionally load SMIRK per-frame pose (yaw, pitch, roll) from .pt file
        smirk_poses = None
        if self.has_smirk and meta.get('smirk') is not None:
            try:
                smirk_data = torch.load(meta['smirk'], map_location='cpu')
                smirk_poses = smirk_data['pose_params']  # (N, 3)
            except Exception:
                smirk_poses = None

        # Align length across all modalities so frame indices stay valid
        min_len = min(len(frame_paths), len(landmarks))
        if smirk_poses is not None:
            min_len = min(min_len, len(smirk_poses))
        if min_len < 2:
        ...
        # When SMIRK data is available, pick the most-frontal frame in the clip
        # as the reference (image_0).  "Most frontal" = minimum |yaw|+|pitch|+|roll|.
        # This gives a stable, near-zero target for the head-stabilisation loss
        # and makes the training signal consistent across iterations.
        # When SMIRK data is absent, fall back to a random reference frame.
        if smirk_poses is not None:
            pose_magnitudes = smirk_poses[:min_len].abs().sum(dim=-1)   # (min_len,)
            f_id0 = int(pose_magnitudes.argmin().item())
        else:
            f_id0 = int(np.random.randint(min_len))

        # Driving frame: random, but must be different from the reference
        candidates = [i for i in range(min_len) if i != f_id0]
        f_id1 = int(np.random.choice(candidates))
        ...
        # SMIRK pose for both frames.
        # pose_1 → supervises MotionEncoder (Step 1)
        # pose_0 → NOT used as the stabilisation target any more (see train.py);
        #          kept in the batch for logging / future reference.
        if smirk_poses is not None:
            pose_0 = smirk_poses[f_id0].float()        # (3,)  — near-zero by construction
            pose_1 = smirk_poses[f_id1].float()        # (3,)
            pose_valid = True
        else:
            pose_0 = torch.zeros(3)
            pose_1 = torch.zeros(3)
            pose_valid = False
        ...
        # Only present when has_smirk=True so batches are always homogeneous.
        # pose_valid covers both pose_0 and pose_1 (same SMIRK file → same validity).
        if self.has_smirk:
            out["pose_0"] = pose_0                                          # (3,) float
            out["pose_1"] = pose_1                                          # (3,) float
            out["pose_valid"] = torch.tensor(pose_valid, dtype=torch.bool)
        return out
