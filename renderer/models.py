...
class PoseHead(nn.Module):
    """Lightweight MLP that predicts (yaw, pitch, roll) from the 32-dim motion latent.

    Used only during renderer training to supervise head-pose encoding inside the
    motion latent.  Not called during inference — does not affect the rendering path.
    """
    def __init__(self, mot_dim: int = 32, hidden_dim: int = 64):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(mot_dim, hidden_dim),
            nn.LeakyReLU(0.2),
...
class IMTRenderer(nn.Module):
    def __init__(self, args):
        ...
        # Pose supervision head — randomly initialized; safe to load pre-trained
        # checkpoints that do not contain pose_head weights (strict=False is used
        # everywhere).  Not called during inference.
        self.pose_head = PoseHead(mot_dim=32, hidden_dim=64)
