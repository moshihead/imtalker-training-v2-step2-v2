...
class IMFSystem(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── Step 1 loss: supervise MotionEncoder so t_1 encodes pose explicitly ──
        # Gradient path: pose_loss → pose_head → t_1 → MotionEncoder
        pose_loss = None
        if "pose_1" in batch and self.args.loss_pose > 0.0:
            valid = batch["pose_valid"]                         # (B,) bool
            if valid.any():
                pose_pred   = self.gen.pose_head(t_1[valid])               # (V, 3)
                pose_target = batch["pose_1"][valid].to(t_1.device)        # (V, 3)
                pose_loss   = F.mse_loss(pose_pred, pose_target)

        # ── Step 2 loss: head stabilisation ──────────────────────────────────
        # ta_10 is the adapted motion that drives the rendered output pred.
        # Forcing pose_head(ta_10) → pose_0 teaches IdentidyAdaptive to carry
        # the *reference* frame's head pose into the rendering instead of the
        # driving frame's head pose.  Lip/expression dimensions in ta_10 are
        # not constrained here, so lip-sync is preserved.
        #
        # Gradient path:
        #   head_stable_loss → pose_head → ta_10 → IdentidyAdaptive → t_1 + id_0
        head_stable_loss = None
        if "pose_0" in batch and self.args.loss_head_stable > 0.0:
            valid = batch["pose_valid"]                         # (B,) bool
            if valid.any():
                stable_pred   = self.gen.pose_head(ta_10[valid])            # (V, 3)
                # Target is always the perfect frontal pose (0, 0, 0).
                # dataset.py already selects the most-frontal clip frame as
                # image_0, so pose_0 ≈ 0 in practice — but using the hard
                # zero target is more consistent and drives the loss to push
                # ta_10 toward a truly neutral head pose regardless of how
                # frontal the selected reference frame actually was.
                stable_target = torch.zeros_like(stable_pred)               # (V, 3) all zeros
                head_stable_loss = F.mse_loss(stable_pred, stable_target)
        ...
        if pose_loss is not None:
            total_g_loss = total_g_loss + self.args.loss_pose * pose_loss
        if head_stable_loss is not None:
            total_g_loss = total_g_loss + self.args.loss_head_stable * head_stable_loss
        ...
        if pose_loss is not None:
            log_dict['train/pose_loss'] = pose_loss
        if head_stable_loss is not None:
            log_dict['train/head_stable_loss'] = head_stable_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loss_pose", type=float, default=0.5,
                        help="Weight for SMIRK pose supervision on the motion latent. "
                             "Set to 0.0 to disable. Has no effect when SMIRK data is absent.")
    parser.add_argument("--loss_head_stable", type=float, default=3.0,
                        help="Weight for head stabilisation loss on the adapted motion ta_10. "
                             "Forces adapted motion to predict the perfect frontal pose (0,0,0). "
                             "Raised from 1.0 → 3.0 to overcome reconstruction-loss dominance. "
                             "Set to 0.0 to disable. Has no effect when SMIRK data is absent.")
